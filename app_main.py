...
Tabs:
  ...
  13. 🧑‍⚖️ Reviewer Agent     — Draft peer review reports
"""
...
/* Main hero */
.ksm-hero {
  background: linear-gradient(135deg, rgba(10,18,40,.95), rgba(14,22,38,.92));
...
.ksm-kicker {
  font-size: .78rem; letter-spacing: .14em; color:#58A6FF; text-transform:uppercase;
}
.ksm-title {
  font-size: 2.55rem; font-weight: 800; margin: .35rem 0 .4rem 0; line-height: 1.05;
}
.ksm-title em { color:#58A6FF; font-style: italic; }
.ksm-sub {
  color:#A5B4C3; font-size: .98rem; line-height:1.6; margin:0;
}
...
.stTabs [aria-selected="true"] {
  ...
  border-color: rgba(255,95,95,.38) !important;
}
...
.ksm-titlebox .t1 { font-size:1.9rem; font-weight:700; margin:0; }
.ksm-titlebox .t2 { color:#8B949E; margin-top:.35rem; font-size:.93rem; }

div[data-testid="stVerticalBlock"] div.stChatMessage {
  border-radius: 12px;
}
</style>
...
st.markdown("""
<div class="ksm-hero">
  <div class="ksm-kicker">Yeshiva University · Katz School of Science and Health</div>
  <div class="ksm-title">KatzScholar<em>Mind</em></div>
  <p class="ksm-sub">A Katz-native research and academic navigation assistant for graduate students<br>
  From topic discovery to faculty connection in one grounded workflow</p>
</div>
""", unsafe_allow_html=True)
...
with tab_chat:
    st.markdown("""
    <div class="ksm-titlebox">
      <div class="t1">💬 PaperBot — AI Research Assistant</div>
      <div class="t2">Grounded in the current paper set. Ask for the latest paper, novelty directions, positioning advice, or reading order.</div>
    </div>
    """, unsafe_allow_html=True)
...
with tab_katzbot:
    st.markdown("""
    <div class="ksm-titlebox">
      <div class="t1">🎓 KatzBot — Yeshiva University AI Assistant</div>
      <div class="t2">Grounded in the Katz School website. Ask about programs, faculty, admissions, tuition, events, and research.</div>
    </div>
    """, unsafe_allow_html=True)
    ...
    # Chat display
    katz_box = st.container(height=520, border=True)
    with katz_box:
        for msg in st.session_state.katz_chat:
            with st.chat_message(msg["role"]):
                ...
                if msg.get("sources"):
                    ...
                    if links:
                        st.markdown(
                            "<div style='margin-top:.35rem;font-size:.78rem;color:#8B949E'>Sources: " + " · ".join(links) + "</div>",
                            unsafe_allow_html=True,
                        )
                if msg.get("faculty"):
